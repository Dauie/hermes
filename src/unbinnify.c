...
void			get_resultclist_from_binnlist(binn *list, t_node **clist)
{
	int			i;
	int			cnt;
	t_result	*res;
...
void			unbinnify_resultset(t_resultset *set, binn *obj)
{
	binn		*results;
	...
	{
		get_resultclist_from_binnlist(results, &set->results);
	}
}
