...
int				h_custom_random_payload(t_mgr *mgr, char *input)
{
	...
	if (!input)
		return (hermes_error(EXIT_FAILURE, 1, "--data-length not specified"));
	if ((len = atoi(input)) <= 0 || len > MAX_PKT_DATA)
		return (hermes_error(EXIT_FAILURE, 1, "bad data-length for random payload"));
	if ((devrand = fopen("/dev/random", "r")) == NULL)
		return (hermes_error(EXIT_FAILURE, 2, "fopen()", strerror(errno)));
...
{
	if (!input)
		return (hermes_error(EXIT_FAILURE, 1, "--data not specified"));
